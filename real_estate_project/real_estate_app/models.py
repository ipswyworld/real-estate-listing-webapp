...
class Property(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.images.exists():
            logger.debug("Images exist, creating thumbnail.")
            self.create_thumbnail()
        else:
            logger.debug("No images found, skipping thumbnail creation.")
    ...
    def create_thumbnail(self):
        ...
        image_path = self.images.first().image.path
        ...
        try:
            with Image.open(image_path) as img:
                img.thumbnail((250, 167))  # Create a smaller thumbnail
                img.save(thumb_path, img.format)
                logger.debug(f"Thumbnail created at {thumb_path}")
                
                # Update the thumbnail field and save the model
...
class Client(models.Model):
    real_estate_app_client_id = models.AutoField(primary_key=True)
    real_estate_app_client_name = models.CharField(max_length=100)
    real_estate_app_client_email = models.EmailField()
...
class Inquiry(models.Model):
    ...
    real_estate_app_inquiry_client = models.ForeignKey(Client, on_delete=models.CASCADE)
...
class Transaction(models.Model):
    ...
    real_estate_app_transaction_client = models.ForeignKey(Client, on_delete=models.CASCADE)
...
class Review(models.Model):
    ...
    real_estate_app_review_client = models.ForeignKey(Client, on_delete=models.CASCADE)
